...
function FizzRun()
	...
	target3 = GetWeakEnemy('TRUE',600)

	if FizzConfig.combo then Combo() end
    if FizzConfig.ult then Ult() end
	if FizzConfig.ignite then ignite() end
	end
end

	FizzConfig = scriptConfig("Fizz Config", "fizzconfig")
	FizzConfig:addParam("combo", "Combo", SCRIPT_PARAM_ONKEYDOWN, false, 88)
	FizzConfig:addParam("ult", "Ultimate", SCRIPT_PARAM_ONKEYDOWN, false, 89)
	FizzConfig:addParam("ignite", "Auto Ignite", SCRIPT_PARAM_ONOFF, true)
	FizzConfig:addParam("useItems", "Use Items", SCRIPT_PARAM_ONOFF, true)
	FizzConfig:addParam("edodge", "E CC Dodge", SCRIPT_PARAM_ONOFF, true)
	FizzConfig:addParam("drawskillshot", "Draw Skillshots", SCRIPT_PARAM_ONOFF, true)
	FizzConfig:addParam("dodgekillshot", "Dodge Skillshots", SCRIPT_PARAM_ONOFF, true)
	FizzConfig:addParam("QSS", "Auto QSS", SCRIPT_PARAM_ONOFF, true)
	FizzConfig:addParam("BlockInputDelay", "BlockInput Delay", SCRIPT_PARAM_NUMERICUPDOWN, 500, 112, 0, 2500, 250)
	FizzConfig:permaShow("combo")
	FizzConfig:permaShow("ult")
	FizzConfig:permaShow("ignite")
	FizzConfig:permaShow("useItems")
	FizzConfig:permaShow("edodge")
	FizzConfig:permaShow("drawskillshot")
	FizzConfig:permaShow("dodgekillshot")
	FizzConfig:permaShow("QSS")
	FizzConfig:permaShow("BlockInputDelay")
	
function Combo()
	if target ~= nil then
		if FizzConfig.useItems then
			UseAllItems(target)
		end	
		if QRDY == 1 then
			CastSpellTarget("Q",target)
		end
		if WRDY == 1 and GetDistance(target) < myHero.range+50 then
...
function Ult()
	if target2 ~= nil then
		CastSpellXYZ('R',GetFireahead(target2,2,14))
	end
end
				...
				if unit ~= nil and spell ~= nil and unit.name == enemy.name then
					for _, spells in pairs(SpellNames) do
						if spell.name == spells.name and spell.target ~= nil and spell.target.name == myHero.name then
							if FizzConfig.edodge and ERDY == 1 then
								CastSpellXYZ("E",myHero.x,0,myHero.z)
							end
						end
					end     
					if spell.name == "SoulShackles" and GetDistance(spell) < 600 and FizzConfig.edodge then
						shieldtick = GetTick()
						if GetTick() - shieldtick > 2000 and GetDistance(spell) < 1050 then
							CastSpellXYZ("E",myHero.x,0,myHero.z)
						end
					if spell.name == "FallenOne" and FizzConfig.edodge then
						shieldtick = GetTick()
			...
			for i=1, #skillshotArray, 1 do
				...
				if spell.name == skillshotArray[i].name then
					...
					elseif skillshotArray[i].type == 5 then
						...
						dodgeaoe(unit, P2, dodgeradius)
					elseif skillshotArray[i].type == 11 then
						skillshotArray[i].p1x = unit.x
						skillshotArray[i].p1y = unit.y
						skillshotArray[i].p1z = unit.z
						skillshotArray[i].p2x = unit.x + (maxdist)/calc*(P2.x-unit.x)
						skillshotArray[i].p2y = P2.y
						skillshotArray[i].p2z = unit.z + (maxdist)/calc*(P2.z-unit.z)
						dodgelinepassevil(unit, P2, dodgeradius, maxdist)
					elseif skillshotArray[i].type == 22 then
						skillshotArray[i].px = P2.x
						skillshotArray[i].py = P2.y
						skillshotArray[i].pz = P2.z
						dodgelinepointevil(unit, P2, dodgeradius)
					elseif skillshotArray[i].type == 33 then
						skillshotArray[i].skillshotpoint = calculateLineaoe(unit, P2, maxdist)
						if skillshotArray[i].name ~= "SummonerClairvoyance" then
							dodgeaoeevil(unit, P2, dodgeradius)
						end
					elseif skillshotArray[i].type == 44 then
						skillshotArray[i].px = unit.x + (maxdist)/calc*(P2.x-unit.x)
						skillshotArray[i].py = P2.y
						skillshotArray[i].pz = unit.z + (maxdist)/calc*(P2.z-unit.z)
						dodgelinepassevil(unit, P2, dodgeradius, maxdist)
					elseif skillshotArray[i].type == 55 then
						skillshotArray[i].skillshotpoint = calculateLineaoe2(unit, P2, maxdist)
						dodgeaoeevil(unit, P2, dodgeradius)
					end
				end
...
function OnCreateObj(obj)
	if obj ~= nil then
		if (GetDistance(myHero, obj)) < 100 and FizzConfig.QSS then
...
function ignite()
	...
	if target3 ~= nil then
		if myHero.SummonerD == 'SummonerDot' then
			if target3.health < damage then
				CastSpellTarget('D',target3)
			end
		end
		if myHero.SummonerF == 'SummonerDot' then
			if target3.health < damage then
				CastSpellTarget('F',target3)
			end
		end
	end
end
...
function OnDraw()
	if QRDY == 1 then
		CustomCircle(550,6,2,myHero)
	end
	if RRDY == 1 then
		CustomCircle(1200,6,3,myHero)
	end
	if target ~= nil then
...
function dodgeaoe(pos1, pos2, radius)
	...
	if calc < radius and FizzConfig.dodgekillshot == true then
		if FizzConfig.combo == false and FizzConfig.ult == false then
			if FizzConfig.BlockInputDelay ~= 0 then
				send.block_input(true, FizzConfig.BlockInputDelay)
			end
...
function dodgeaoeevil(pos1, pos2, radius)
	local calc = (math.floor(math.sqrt((pos2.x-myHero.x)^2 + (pos2.z-myHero.z)^2)))
	local dodgex
	local dodgez
	dodgex = pos2.x + ((radius+150)/calc)*(myHero.x-pos2.x)
	dodgez = pos2.z + ((radius+150)/calc)*(myHero.z-pos2.z)
	if calc < radius then
		if FizzConfig.edodge == true and ERDY == 1 then
			CastSpellXYZ("E",dodgex,0,dodgez)
		elseif (FizzConfig.dodgeskillshot == true and FizzConfig.edodge == false) or (FizzConfig.dodgeskillshot == true and FizzConfig.edodge == true and ERDY == 0) then
			if FizzConfig.combo == false and FizzConfig.ult == false then
				if FizzConfig.BlockInputDelay ~= 0 then
					send.block_input(true, FizzConfig.BlockInputDelay)
				end
...
function dodgelinepoint(pos1, pos2, radius)
	...
	if perpendicular < radius and calc1 < calc4 and calc2 < calc4 and FizzConfig.dodgekillshot == true then
		if FizzConfig.combo == false and FizzConfig.ult == false then
			if FizzConfig.BlockInputDelay ~= 0 then
				send.block_input(true, FizzConfig.BlockInputDelay)
			end
...
function dodgelinepointevil(pos1, pos2, radius)
	local calc1 = (math.floor(math.sqrt((pos2.x-myHero.x)^2 + (pos2.z-myHero.z)^2)))
	local calc2 = (math.floor(math.sqrt((pos1.x-myHero.x)^2 + (pos1.z-myHero.z)^2)))
	local calc4 = (math.floor(math.sqrt((pos1.x-pos2.x)^2 + (pos1.z-pos2.z)^2)))
	local calc3
...
function dodgelinepassevil(pos1, pos2, radius, maxDist)
	local pm2x = pos1.x + (maxDist)/(math.floor(math.sqrt((pos1.x-pos2.x)^2 + (pos1.z-pos2.z)^2)))*(pos2.x-pos1.x)
	local pm2z = pos1.z + (maxDist)/(math.floor(math.sqrt((pos1.x-pos2.x)^2 + (pos1.z-pos2.z)^2)))*(pos2.z-pos1.z)
	local calc1 = (math.floor(math.sqrt((pm2x-myHero.x)^2 + (pm2z-myHero.z)^2)))
	local calc2 = (math.floor(math.sqrt((pos1.x-myHero.x)^2 + (pos1.z-myHero.z)^2)))
...
function Skillshots()
	...
	if FizzConfig.drawskillshot == true then
		for i=1, #skillshotArray, 1 do
...
function LoadTable()
	...
	for i=0, iCount, 1 do
		...
		if 1==1 or skillshotplayerObj.name == "Quinn" then
			table.insert(skillshotArray,{name= "QuinnQMissile", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1025, type = 11, radius = 40, color= coloryellow, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
			skillshotcharexist = true -- BLIND
		end
		...
		if 1==1 or skillshotplayerObj.name == "Zac" then
			...
			table.insert(skillshotArray,{name= "ZacE", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1550, type = 33, radius = 200, color= colorcyan, time = 2, isline = false, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
			skillshotcharexist = true -- KNOCKBACK
		end
		if 1==1 or skillshotplayerObj.name == "Syndra" then
			...
			table.insert(skillshotArray,{name= "SyndraE", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 650, type = 11, radius = 100, color= coloryellow, time = 0.5, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
			...
			skillshotcharexist = true -- STUN
		end
		if 1==1 or skillshotplayerObj.name == "Jayce" then
			table.insert(skillshotArray,{name= "jayceshockblast", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1470, type = 11, radius = 100, color= colorcyan, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
			skillshotcharexist = true -- Jayce QE
		end
		if 1==1 or skillshotplayerObj.name == "Nami" then
			table.insert(skillshotArray,{name= "NamiQ", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 875, type = 33, radius = 200, color= coloryellow, time = 1, isline = false, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
			table.insert(skillshotArray,{name= "NamiR", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 2550, type = 11, radius = 350, color= colorcyan, time = 3, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
			skillshotcharexist = true -- STUN + KNOCKUP
		end
		if 1==1 or skillshotplayerObj.name == "Vi" then
			table.insert(skillshotArray,{name= "ViQ", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 900, type = 11, radius = 150, color= coloryellow, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
			skillshotcharexist = true -- KNOCKBACK
		end
			if 1==1 or skillshotplayerObj.name == "Thresh" then
			table.insert(skillshotArray,{name= "ThreshQ", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1100, type = 11, radius = 100, color= coloryellow, time = 1.5, isline = false, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
			skillshotcharexist = true -- GRAB
		end
		...
		if 1==1 or skillshotplayerObj.name == "Elise" then
			table.insert(skillshotArray,{name= "EliseHumanE", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1075, type = 11, radius = 100, color= colorcyan, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
			skillshotcharexist = true -- STUN
		end
		...
		if 1==1 or skillshotplayerObj.name == "Ahri" then
			...
			table.insert(skillshotArray,{name= "AhriSeduce", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 975, type = 11, radius = 150, color= colorcyan, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0})
		skillshotcharexist = true -- CHARM
		end
		if 1==1 or skillshotplayerObj.name == "Amumu" then
			table.insert(skillshotArray,{name= "BandageToss", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1100, type = 11, radius = 150, color= colorcyan, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0})
		skillshotcharexist = true -- GRAB
		end
		...
		if 1==1 or skillshotplayerObj.name == "Ashe" then
			table.insert(skillshotArray,{name= "EnchantedCrystalArrow", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 50000, type = 44, radius = 120, color= colorcyan, time = 4, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0})
		skillshotcharexist = true -- STUN
		end
		if 1==1 or skillshotplayerObj.name == "Blitzcrank" then
			table.insert(skillshotArray,{name= "RocketGrabMissile", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 925, type = 11, radius = 150, color= colorcyan, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0})
		skillshotcharexist = true -- GRAB
		end
		...
		if 1==1 or skillshotplayerObj.name == "Chogath" then
			table.insert(skillshotArray,{name= "Rupture", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 950, type = 33, radius = 275, color= coloryellow, time = 1.5, isline = false, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- KNOCKUP
		end
		...
		if 1==1 or skillshotplayerObj.name == "Heimerdinger" then
			table.insert(skillshotArray,{name= "CH1ConcussionGrenade", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 950, type = 33, radius = 225, color= coloryellow, time = 1.5, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- STUN / BLIND
		end
		...
		if 1==1 or skillshotplayerObj.name == "Fizz" then
			table.insert(skillshotArray,{name= "FizzMarinerDoom", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1275, type = 22, radius = 100, color= colorcyan, time = 1.5, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- KNOCKUP
		end
		...
		if 1==1 or skillshotplayerObj.name == "Gragas" then
			...
			table.insert(skillshotArray,{name= "GragasExplosiveCask", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1050, type = 33, radius = 400, color= coloryellow, time = 1.5, isline = false, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- KNOCKBACK
		end
		...
		if 1==1 or skillshotplayerObj.name == "Janna" then
			table.insert(skillshotArray,{name= "HowlingGale", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1700, type = 11, radius = 100, color= colorcyan, time = 2, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- KNOCKUP
		end
		...
		if 1==1 or skillshotplayerObj.name == "LeeSin" then
			...
			table.insert(skillshotArray,{name= "BlindMonkRKick", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1200, type = 11, radius = 100, color= colorcyan, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- KNOCKBACK
		end
		if 1==1 or skillshotplayerObj.name == "Leona" then
			table.insert(skillshotArray,{name= "LeonaZenithBladeMissile", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 700, type = 11, radius = 150, color= colorcyan, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- STUN
		end
		if 1==1 or skillshotplayerObj.name == "Lux" then
			table.insert(skillshotArray,{name= "LuxLightBinding", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1175, type = 11, radius = 150, color= colorcyan, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		...
		if 1==1 or skillshotplayerObj.name == "Malzahar" then
			table.insert(skillshotArray,{name= "AlZaharCalloftheVoid", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 900, type = 33, radius = 100 , color= coloryellow, time = 1, isline = false, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		...
		skillshotcharexist = true -- SILENCE
		end
		...
		if 1==1 or skillshotplayerObj.name == "Morgana" then
			table.insert(skillshotArray,{name= "DarkBindingMissile", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1300, type = 11, radius = 90, color= colorcyan, time = 1.5, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		...
		if 1==1 or skillshotplayerObj.name == "Nautilus" then
			table.insert(skillshotArray,{name= "NautilusAnchorDrag", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 950, type = 11, radius = 150, color= colorcyan, time = 1.5, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- GRAB
		end
		if 1==1 or skillshotplayerObj.name == "Nidalee" then
			table.insert(skillshotArray,{name= "JavelinToss", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1500, type = 11, radius = 150, color= colorcyan, time = 1.5, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0  })
		skillshotcharexist = true -- Nidalee!
		end
		...
		if 1==1 or skillshotplayerObj.name == "Shen" then
			table.insert(skillshotArray,{name= "ShenShadowDash", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 600, type = 22, radius = 150, color= colorcyan, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0  })
		skillshotcharexist = true -- TAUNT
		end
		...
		if 1==1 or skillshotplayerObj.name == "Sona" then
			table.insert(skillshotArray,{name= "SonaCrescendo", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1000, type = 11, radius = 150, color= colorcyan, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- DANCE
		end
		if 1==1 or skillshotplayerObj.name == "Sejuani" then
			table.insert(skillshotArray,{name= "SejuaniGlacialPrison", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1150, type = 11, radius = 150, color= colorcyan, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- STUN
		end
		if 1==1 or skillshotplayerObj.name == "Swain" then
			table.insert(skillshotArray,{name= "SwainShadowGrasp", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 900, type = 33, radius = 265 , color= coloryellow, time = 1.5, isline = false, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- STUN
		end
		...
		if 1==1 or skillshotplayerObj.name == "Urgot" then
			...
			table.insert(skillshotArray,{name= "UrgotPlasmaGrenade", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 950, type = 33, radius = 300, color= coloryellow, time = 1, isline = false, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0  })
		skillshotcharexist = true -- Urgot E
		end
		...
		skillshotcharexist = true -- Ziggs R
		end
		if 1==1 or skillshotplayerObj.name == "Zyra" then
			...
			table.insert(skillshotArray,{name= "ZyraGraspingRoots", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 1100, type = 11, radius = 90, color= colorcyan, time = 2, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- SNARE
		end
		if 1==1 or skillshotplayerObj.name == "Diana" then
			table.insert(skillshotArray,{name= "DianaArc", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 900, type = 33, radius = 205, color= coloryellow, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
		skillshotcharexist = true -- Diana Q
		end
    end
end

SetTimerCallback("FizzRun")
print("\nVal's Fizz v"..version.."")