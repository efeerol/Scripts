...
if IsChatOpen() == 0 then
     targetq = GetWeakEnemy('PHYS',1500)
        ...
        target4 = GetWeakEnemy('PHYS',925)
		target5 = GetWeakEnemy('TRUE',150)
	    target6 = GetWeakEnemy('MAGIC',600)
        targetignite = GetWeakEnemy('TRUE',600)
		if blitzcrank.Combo then Autoharass() end
        if blitzcrank.shielditems then shielditems()  end
        if blitzcrank.ignite then ignite() end
		if blitzcrank.useItems then useItems() end
end
end
...
function Main()
        ...
        blitzcrank, menu = uiconfig.add_menu('Insane blitzcrank', 200)
        menu.keydown('Combo', 'Grab', Keys.X)       
...
SpellPred(Q,QRDY,myHero,target4,925,2,16,1)
...
SetTimerCallback('blitzcrankRun')