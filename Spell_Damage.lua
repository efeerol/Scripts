...
Aatrox = {
	...
	Lucian = {
		...
		PType = 2,
		QDmgP = "40*Qlvl+40+(15*Qlvl+45)*bad/100",
...
function getDmg(spellname,SDLtarget,SDLplayer,stagedmg,spelllvl)
                    ...
                    elseif (spellname == "TRINITY") then
                            TrueDmg = CalcDamage(SDLtarget,(ad-bad)*2) --(bonus)
                    elseif (spellname == "LICHBANE") then
                            TrueDmg = CalcMagicDamage(SDLtarget,50+.75*ap) --(bonus)
                    elseif (spellname == "LIANDRYS") then
                            TrueDmg = CalcMagicDamage(SDLtarget,.06*thp) --over 3 sec, If their movement is impaired, they take double damage from this effect. Half duration for multi-target or periodic effects
                    ...
                    elseif (spellname == "ICEBORN") then
                            TrueDmg = CalcDamage(SDLtarget,(ad-bad)*1.25) --(bonus)
                    ...
                    elseif (spellname == "RUINEDKING") then
                            TrueDmg = math.max((CalcDamage(SDLtarget,.05*thp))*(stagedmg1+stagedmg3),CalcDamage(SDLtarget,math.max(.15*tmhp,100))*stagedmg2) --stage1:Passive. stage2:Active. stage3: stage1
