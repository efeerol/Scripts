...
function RyzeRun()
    ...
    if RyzeConfig.PushFarm then PushFarm() end
	end
...
RyzeConfig:addParam("PushFarm", "Push Farm (Z)", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("Z"))
...
function combo()
	if target ~= nil then
		...
		if WRDY == 1 and GetDistance(myHero, target) < 600 then
			CastSpellTarget('W', target)
		end
		if ERDY == 1 and GetDistance(myHero, target) < 600 then
