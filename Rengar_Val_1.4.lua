...
local skillingOrder = {Rengar = {Q,W,E,Q,Q,R,Q,E,Q,E,R,E,E,W,W,R,W,W},}
------- END -------
...
function RengarRun()
	...
	if RengarConfig.drawcircles then RengarDraw() end
	if RengarConfig.drawskillshots then Skillshots() end
	if RengarConfig.autolevel then Autolevel() end
	if RengarConfig.combo and timer3 == 0 then combo() end
	if RengarConfig.ignite then ignite() end
...
function combo()
	...
	if target ~= nil then
		if myHero.mana < 5 and ERDY == 1 then			
			CastSpellTarget('E', target)
		end
		if myHero.mana < 5 and WRDY == 1 then
			if GetDistance(target) < 500 then
				CastSpellXYZ('W',myHero.x,0,myHero.z)
			end
		end
		if timer == 0 and ERDY == 0 and WRDY == 0 then
			AttackTarget(target)
		end
	end
	if targetaa ~= nil then
		if RengarConfig.useItems then
			UseAllItems(targetaa)
		end
	end
	if RengarConfig.movement and timer == 0 and timer2 == 0 then
		if targetaa == nil then
			MoveToMouse()
...
function Timer()
	...
	if targetaa ~= nil and timer ~= 0 then
...
function OnProcessSpell(unit,spell)
    if unit ~= nil and spell ~= nil and unit.charName == myHero.charName then
 		if spell.name == "RengarBasicAttack" or spell.name == "RengarBasicAttack2" or spell.name == "RengarBasicAttack3" or spell.name == "RengarCritAttack" or spell.name == "RengarQ" and targetaa ~= nil and spell.targetaa and QRDY == 1 and (RengarConfig.combo or RengarConfig.autoq) then
			timer = GetTickCount()
		end
		if spell.name == "RengarBasicAttack" or spell.name == "RengarBasicAttack2" or spell.name == "RengarBasicAttack3" or spell.name == "RengarCritAttack" or spell.name == "RengarQ" and targetaa ~= nil and spell.targetaa and timer == 0 then
			timer2 = GetTickCount()
		end
	end
...
function dodgeaoe(pos1, pos2, radius)
	...
	if calc < radius and RengarConfig.dodgeskillshots then
		if RengarConfig.forceaoeskillshots then timer3 = GetTickCount() end
...
function dodgelinepoint(pos1, pos2, radius)
	...
	if perpendicular < radius and calc1 < calc4 and calc2 < calc4 and RengarConfig.dodgeskillshots then
		if RengarConfig.forceskillshots then timer3 = GetTickCount() end
        MoveToXYZ(dodgex,0,dodgez)
	end
end
...
SetTimerCallback('RengarRun')
print("\nVal's Rengar v"..version.."\n")