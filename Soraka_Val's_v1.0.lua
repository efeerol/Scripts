...
local version = '1.0'
local skillingOrder = {Soraka = {W,E,W,E,W,R,W,E,W,E,R,E,Q,Q,Q,R,Q,Q},}
---------------------------
local wUsedAt = 0
local vUsedAt = 0
local mUsedAt = 0
...
function Main()
	if IsChatOpen() == 0 and myHero.name == "Soraka" then
		SetVariables()
		RedElixir()
		Wards()
		if SorakaHotkeys.SmartWard then Smart_wards() end
		if SorakaHotkeys.AutoHeal then AutoHeal() end
		if SorakaHotkeys.AutoSilence then AutoSilence() end
		if SorakaHotkeys.AutoUlt then AutoUlt() end
		if SorakaHotkeys.Autolevel then Autolevel() end
		Skillshots()
	end
end

	SorakaHotkeys, menu = uiconfig.add_menu('1. Soraka hotkeys', 250)
	menu.keytoggle('AutoHeal', 'Auto-Heal', Keys.F1, true)
	menu.keytoggle('AutoSilence', 'Auto-Silence', Keys.F2, true)
	menu.keytoggle('AutoUlt', 'Auto-Ult', Keys.F3, true)
	menu.keydown('SmartWard', 'Smart-Ward', Keys.T)
	menu.checkbutton('Autolevel', 'Autolevel', true)
	menu.permashow('AutoHeal')
	menu.permashow('AutoSilence')
	menu.permashow('AutoUlt')
	menu.permashow('SmartWard')
	menu.permashow('Autolevel')
	
	CfgPotions, menu = uiconfig.add_menu('2. Potions', 200)
...
function AutoHeal()
	for i = 1, objManager:GetMaxHeroes() do
		...
		if (ally ~= nil and ally.team == myHero.team and ally.visible == 1 and ally.dead == 0) then
			local heal = (myHero.SpellLevelW*50+20+myHero.ap*.35)+(((ally.maxHealth-ally.health)/50)*((myHero.SpellLevelW*50+20+myHero.ap*.35)/100))
			if ally.health<(ally.maxHealth-heal) then
				SpellTarget(W,WRDY,myHero,ally,750)
			end
		end
	end
end
...
function AutoSilence()
	for i = 1, objManager:GetMaxHeroes() do
		...
		if (enemy ~= nil and enemy.team ~= myHero.team and enemy.visible == 1 and enemy.dead == 0) then
			if enemy.name == 'Ahri' or enemy.name == 'Akali' or enemy.name == 'Anivia' or enemy.name == 'Annie' or enemy.name == 'Brand' or enemy.name == 'Cassiopeia' or enemy.name == 'Diana' or enemy.name == 'Elise' or enemy.name == 'Evelynn' or enemy.name == 'FiddleSticks' or enemy.name == 'Fizz' or enemy.name == 'Gragas' or enemy.name == 'Heimerdinger' or enemy.name == 'Karma' or enemy.name == 'Karthus' or enemy.name == 'Kassadin' or enemy.name == 'Katarina' or enemy.name == 'Kennen' or enemy.name == 'Leblanc' or enemy.name == 'Lissandra' or enemy.name == 'Lulu' or enemy.name == 'Lux' or enemy.name == 'Malzahar' or enemy.name == 'Mordekaiser' or enemy.name == 'Morgana' or enemy.name == 'Nami' or enemy.name == 'Orianna' or enemy.name == 'Rumble' or enemy.name == 'Ryze' or enemy.name == 'Sona' or enemy.name == 'Soraka' or enemy.name == 'Swain' or enemy.name == 'Syndra' or enemy.name == 'TwistedFate' or enemy.name == 'Veigar' or enemy.name == 'Vladimir' or enemy.name == 'Xerath' or enemy.name == 'Ziggs' or enemy.name == 'Zilean' or enemy.name == 'Zyra' then
				SpellTarget(E,ERDY,myHero,enemy,725)
			end
		end
	end
end
...
function AutoUlt()
	for i = 1, objManager:GetMaxHeroes() do
		local ally = objManager:GetHero(i)
		if (ally ~= nil and ally.team == myHero.team and ally.visible == 1 and ally.dead == 0) then
			if ally.health<(ally.maxHealth/4) then
				SpellXYZ(R,RRDY,myHero,myHero,1000,myHero.x,myHero.z)
			end
		end
	end
end
...
function SetVariables()
	if myHero.SpellTimeQ > 1.0 and GetSpellLevel('Q')~=0 and myHero.mana > (20+(myHero.SpellLevelQ*10)) then
	QRDY = 1
	else QRDY = 0
	end
	if myHero.SpellTimeW > 1.0 and GetSpellLevel('W')~=0 and myHero.mana > (60+(myHero.SpellLevelW*20))then
	WRDY = 1
	else WRDY = 0
	end
...
local wardslot=nil
local items = {3340,3350,2050,3154,2044,2045,2049}
wardSpots = {
        ...
        { x = 11912,   y = 10,     z = 3850},     -- OVERWALL1
        { x = 9835,    y = 52,     z = 3167},     -- OVERWALL2
        { x = 2140,    y = -64,    z = 10709},    -- OVERWALL3
        { x = 4170,    y = 27,     z = 11105},    -- OVERWALL4
}

function Smart_wards()
    if SorakaHotkeys.SmartWard then
        keypressed=true
...
function Wards()
	if SorakaHotkeys.SmartWard == false and keypressed == true then
        for i,wardSpot in pairs(wardSpots) do
			if GetDistance(wardSpot, mousePos) <= 250 then
				for i,item in pairs(items) do
					wardslots = GetInventorySlot(item)
					if wardslots ~= nil then
						place(item,wardslots,wardSpot.x,wardSpot.y,wardSpot.z)
					end
				end
			end
		end
...
function place(warditem,wardslot,placex,placey,placez)
	if warditem~=nil and wardslot~=nil and placex~=nil and placey~=nil and placez~=nil then
		UseItemLocation(warditem,placex,placey,placez)
	end
end
--------- Smart Wards ---------
