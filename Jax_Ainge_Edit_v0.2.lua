...
----- CONFIG: -----
local delay = 142 --140
local ping = 40--40-185
local dodgedelay = 500  --500
local Edelay = 2000  --2000
local Qdelay = Edelay
local lasthitdelay = 1000  --1000
--local skillingOrder = {Jax = {Q,W,E,Q,Q,R,Q,E,Q,E,R,E,E,W,W,R,W,W},}
...
function JaxRun()
	...
	targetaa = GetWeakEnemy('PHYS',AArange) --AArange
	targetignite = GetWeakEnemy('TRUE',650)
	
	Cooldown_Handling()
	Timer()
	
	if JaxConfig.drawcircles then JaxDraw() end
	if JaxConfig.drawskillshots then Skillshots() end
	--if JaxConfig.autolevel then Autolevel() end
	if JaxConfig.combo and timerdodge == 0 then combo() end
	if JaxConfig.ignite then ignite() end
	--if JaxConfig.espell then Espell() end
	
	if debugmode then DrawText("Delay:" .. ((delay+ping)/myHero.attackspeed),70,100,0xFF00EE00) end--10横.100竖
end
end

	JaxConfig, menu = uiconfig.add_menu('Jax Config', 200) --这里是菜单，200是长度、 true是真 False是假
    menu.keydown('combo', 'Combo', Keys.X)
	--menu.keydown('espell', 'E-Spell', Keys.Z)
	menu.checkbutton('ignite', 'Auto-Ignite', true)
	menu.checkbutton('autow', 'Auto-W after AA', true)
	menu.checkbutton('usee', 'Auto-Stun', false)
	menu.checkbutton('user', 'Auto-Ult', false)
...
function combo()
	if target ~= nil then
		if JaxConfig.useItems then
			UseAllItems(target)
		end
		if timerreset == 0 and timerAA == 0 then
			if QRDY == 1 and GetDistance(target) > AArange and GetDistance(target) < 650 then   --当目标距离大于普攻范围，小于700范围。使用Q，“AArang`e“删除
				CastSpellTarget("Q",target)
			end
			if JaxConfig.user and RRDY == 1 and GetDistance(target) <= AArange then
				CastSpellXYZ('R', myHero.x,myHero.y,myHero.z)
			end
			AttackTarget(target)
		end
		if timerE == 0 and timerQ ~= 0 and JaxConfig.usee and ERDY == 1 then
			CastSpellXYZ('E', myHero.x,myHero.y,myHero.z)
		end
		if JaxConfig.usee and attacked == 0 and GetDistance(target) < 180 then
			CastSpellXYZ('E', myHero.x,myHero.y,myHero)
		end
	end
	...
	if JaxConfig.movement and timerreset == 0 and timerAA == 0 then 
		if target == nil then
			MoveToMouse()
...
function Espell()
	if timerE == 0 and ERDY == 1 and JaxConfig.espell then
		CastSpellXYZ('E', myHero.x,myHero.y,myHero.z)
	end
	if target ~= nil and timerE ~= 0 and JaxConfig.espell and GetDistance(target) < 180 then
		CastSpellXYZ('E', myHero.x,myHero.y,myHero.z)
	end
end

function Timer()
	if GetTickCount() - timerreset > (((delay+ping)/myHero.attackspeed)+ping*2) then    ---"attackspee`d"攻击速度， 尝试最快限度的取消攻击动画 
		timerreset = 0
	end
	if GetTickCount() - timerAA > (((delay+ping)/myHero.attackspeed)+ping*2) then
		timerAA = 0
	end
	if GetTickCount() - timerdodge > dodgedelay then
		timerdodge = 0
	end
	if GetTickCount() - timerQ > Qdelay then
		timerQ = 0
	end
	if GetTickCount() - timerE > Edelay then
		timerE = 0
	end
	if GetTickCount() - lasthit < lasthitdelay then
		attacked = 1
	end
	if GetTickCount() - lasthit > lasthitdelay then
		attacked = 1
	...
	if targetaa ~= nil and timerreset ~= 0 then
		if GetTickCount() - timerreset > ((delay+ping)/myHero.attackspeed) then
			CastSpellTarget('W', targetaa)
			AttackTarget(targetaa)
			timerreset = 0
		end
	end	
	-- if targetaa ~= nil and timerAA ~= 0 then    ---我不知道这是干嘛的，但他跟移动有关。。
		-- if GetTickCount() - timerAA > ((delay+ping)/myHero.attackspeed) then
...
function Cooldown_Handling()
	if myHero.SpellTimeQ > 1.0 and myHero.mana >= 65 then
	QRDY = 1
	else QRDY = 0
	end
	if myHero.SpellTimeW > 1.0 and myHero.mana >= 30 then
	WRDY = 1
	else WRDY = 0
	end
	if myHero.SpellTimeE > 1.0 and myHero.mana >= (70+(GetSpellLevel('E')*5)) then
	ERDY = 1
	else ERDY = 0
	end
...
function JaxDraw()
	if QRDY == 1 then  ---如果Q冷却，1 绿2 红 3 蓝4 紫5 黄
...
function ignite()
	...
	if targetignite ~= nil then
		if myHero.SummonerD == "SummonerDot" then
			if targetignite.health < damage then --目标点燃.生命< 损伤
				CastSpellTarget("D",targetignite) --用D 点燃目标
			end
		end
...
function OnProcessSpell(unit,spell)
    if unit ~= nil and spell ~= nil and unit.charName == myHero.charName then
 		if spell.name == "JaxBasicAttack" or spell.name == "JaxBasicAttack2" or spell.name == "JaxCritAttack"  or spell.name == "jaxrelentlessattack" and targetaa ~= nil and spell.targetaa and WRDY == 1 and (JaxConfig.combo or JaxConfig.autow) then
			timerreset = GetTickCount()
		end
		if spell.name == "JaxBasicAttack" or spell.name == "JaxBasicAttack2" or spell.name == "JaxCritAttack"  or spell.name == "jaxrelentlessattack" and targetaa ~= nil and spell.targetaa and timerreset == 0 then
			timerAA = GetTickCount()
		end
		if spell.name == "JaxLeapStrike" and target ~= nil and spell.target and timerQ == 0 then
			timerQ = GetTickCount()
		end
		if spell.name == "JaxCounterStrike" and timerE == 0 then
			timerE = GetTickCount()
		end
	end
	for i = 1, objManager:GetMaxHeroes() do
		...
		if enemy ~= nil and enemy.team ~= myHero.team then
			if unit ~= nil and spell ~= nil and unit.charName == enemy.charName then
				if string.find(spell.name,"attack") or string.find(spell.name,"Attack") and spell.target ~= nil and spell.target.name == myHero.name then
...
function dodgeaoe(pos1, pos2, radius)
	...
	if calc < radius and JaxConfig.dodgeskillshots then
		if JaxConfig.forceaoeskillshots then timerdodge = GetTickCount() end
...
function dodgelinepoint(pos1, pos2, radius)
	...
	if perpendicular < radius and calc1 < calc4 and calc2 < calc4 and JaxConfig.dodgeskillshots then
		if JaxConfig.forceskillshots then timerdodge = GetTickCount() end
        MoveToXYZ(dodgex,0,dodgez)
	end
end
...
function LoadTable()
	...
	for i=0, iCount, 1 do
		...
		if 1==1 or skillshotplayerObj.name == "Zed" then
			table.insert(skillshotArray,{name= "ZedShuriken", shot=0, lastshot = 0, skillshotpoint = {}, maxdistance = 900, type = 1, radius = 200, color= coloryellow, time = 1, isline = true, p1x =0, p1y =0 , p1z =0 , p2x =0, p2y =0 , p2z =0 })
