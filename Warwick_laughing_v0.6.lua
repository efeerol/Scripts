...
function WarwickRun()
	...
	if WarwickConfig.combo then combo() end
	if WarwickConfig.ignite then ignite() end
    if WarwickConfig.QFarm then QFarm() end
	end
...
WarwickConfig = scriptConfig("Warwick Config", "Warwickconfg")
WarwickConfig:addParam("combo", "Combo (X)", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("X"))
WarwickConfig:addParam("QFarm", "Q Farm (Z)", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("Z"))
WarwickConfig:addParam("useItems", "Use Items", SCRIPT_PARAM_ONKEYTOGGLE, true, 112)
WarwickConfig:addParam("ignite", "Auto Ignite", SCRIPT_PARAM_ONKEYTOGGLE, true, 113)
WarwickConfig:permaShow("QFarm")
WarwickConfig:permaShow("combo")
WarwickConfig:permaShow("useItems")
WarwickConfig:permaShow("ignite")
...
function combo()
	if target2 ~= nil then
		if WarwickConfig.useItems then
			UseAllItems(target)
		end	
	if target ~= nil then		
		...
		if QRDY == 1 and GetDistance(myHero, target) < 400 then
				CastSpellTarget('Q', target2)
		end
	if GetDistance(myHero, target2) < myHero.range then
				AttackTarget(target2)
		end
		end
	if target == nil and WarwickConfig.combo then
	MoveToMouse()
	elseif WarwickConfig.combo then		
	MoveToMouse()
end
end
...
function QFarm()
	...
	else target = GetLowestHealthEnemyMinion(600)
	end
