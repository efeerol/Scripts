...
MordeConfig:addParam('efa', "E Fireahead", SCRIPT_PARAM_NUMERICUPDOWN, 3.2, 187,0,4,0.1)
...
function Run()
	...
	if targetult~=nil then
		...
		twfxW,twfyW,twfzW = GetFireahead(targetult,3.79,0)
		twfaW={x=twfx,y=0,z=twfz}
		
		tefxE,tefyE,tefzE = GetFireahead(targetult,EFA,0)
		tefaE={x=tefx,y=0,z=tefz}
		
		QQ = getDmg('Q',targetult,myHero)*QRDY
		QQQ = getDmg('Q',targetult,myHero,3)*QRDY
		WW = getDmg('W',targetult,myHero)*WRDY
		EE = getDmg('E',targetult,myHero)*ERDY
...
function OnProcessSpell(unit,spell)
	...
	elseif (shield==true) and unit~=nil and myHero.dead~=1 then
	local Q
	local W
	local E
	local R
...
function harass()
	if targetult~=nil then
		...
		if GetD(tefaE)<665 and ERDY==1 then
				CastSpellXYZ('E',bestE(targetult))
		elseif ERDY==1 and target~=nil and GetD(tefa)<665 then
			CastSpellXYZ('E',bestE(target))
		end
		if GetD(targetult)<275 then
			if RRDY==1 and myHero.SpellNameR=="MordekaiserChildrenOfTheGrave" and tu.dead~=1 and tu.health<RR and GetD(tu)<850 then 
				CastSpellTarget('R',tu)
			elseif RRDY==1 and myHero.SpellNameR=="MordekaiserChildrenOfTheGrave" and tu.dead~=1 and tu.health<RRR and ignitedamage>0 and GetD(tu)<850 then
		...
		if WRDY==1 and GetD(twfaW)<300 then
			CastSpellTarget('W',myHero)
		elseif target~=nil and WRDY==1 and GetD(twfa)<300 then
			CastSpellTarget('W',myHero)
				
		end
	
		if (QRDY==0 or GetD(targetult)>=275) and (WRDY==0 or GetD(twfaW)>=300) and (ERDY==0 or GetD(tefaE)>=665) and (RRDY==0 or GetD(targetult)>800) then
	...
	if targetult~=nil then
		...
		elseif ERDY==1 and target~=nil and GetD(tefa)<665 then
			CastSpellXYZ('E',bestE(target))
				...
				if GetD(targetult)<275 then
			if RRDY==1 and myHero.SpellNameR=="MordekaiserChildrenOfTheGrave" and tu.dead~=1 and tu.invulnerable~=1 and tu.visible==1 and tu.health<RR and GetD(tu)<850 then 
				CastSpellTarget('R',tu)
			elseif RRDY==1 and myHero.SpellNameR=="MordekaiserChildrenOfTheGrave" and tu.dead~=1 and tu.invulnerable~=1 and tu.visible==1 and tu.health<RRR and ignitedamage>0 and GetD(tu)<850 then
				CastSummonerIgnite(tu)
				CastSpellTarget('R',tu)
			elseif QRDY==1 then
				CastSpellTarget('Q',myHero)
				AttackTarget(targetult)
			elseif RRDY==1 and myHero.SpellNameR=="MordekaiserChildrenOfTheGrave" and tu.dead~=1 and tu.invulnerable~=1 and tu.visible==1 and myHero.health<25/100*myHero.maxHealth and GetD(tu)<850 then
				CastSummonerIgnite(tu)
				CastSpellTarget('R',tu)
			else
				CastSummonerExhaust(targetult)
		...
		elseif GetD(targetult)<850 then
			if RRDY==1 and myHero.SpellNameR=="MordekaiserChildrenOfTheGrave" and tu.dead~=1 and tu.invulnerable~=1 and tu.visible==1 and tu.health<RR and GetD(tu)<850 then 
				CastSpellTarget('R',tu)
			elseif RRDY==1 and myHero.SpellNameR=="MordekaiserChildrenOfTheGrave" and tu.dead~=1 and tu.invulnerable~=1 and tu.visible==1 and ((tu.health<RRR and ignitedamage>0) or (tu.health<RRR-20 and not runningAway(tu))) and GetD(tu)<850 then 
				CastSummonerIgnite(tu)
				CastSpellTarget('R',tu)
			elseif WRDY==1 and allySurroundedMost()~=nil then
				CastSpellTarget('W',allySurroundedMost())
...
function autoQ()
	if target~=nil and QRDY==1 and GetD(target)<250 then
		CastSpellTarget('Q',myHero)
		AttackTarget(target)
	end
end
...
function autoE()
	if target~=nil and GetD(tefa)<665 and ERDY==1 then
		CastSpellXYZ('E',bestE(target))
	end
end
...
function killsteal()
	if target~=nil then
			...
			if tu~=nil then
				...
				QQclose = getDmg('Q',tu,myHero)*QRDY
				QQQclose = getDmg('Q',tu,myHero,3)*QRDY
				EEclose = getDmg('E',tu,myHero)*ERDY
				if myHero.SpellNameR=="MordekaiserChildrenOfTheGrave" then
...
function lingeringKS()
	if target~=nil then
		...
		if tu~=nil then
			...
			QQclose = getDmg('Q',tu,myHero)*QRDY
			QQQclose = getDmg('Q',tu,myHero,3)*QRDY
			WWclose = getDmg('W',tu,myHero)*WRDY
			EEclose = getDmg('E',tu,myHero)*ERDY
			if myHero.SpellNameR=="MordekaiserChildrenOfTheGrave" then
			RRclose = CalcMagicDamage(tu,(9.5+2.5*GetSpellLevel('R')+2*(math.floor(myHero.ap/100)))*tu.maxHealth/100)*RRDY
...
function dodgelinepass(pos1, pos2, radius, maxDist)
    ...
    dodgez = z4 + ((radius+150)/calc3)*(myHero.z-z4)
    if perpendicular < radius and calc1 < calc4 and calc2 < calc4 then
		if willDie==true and MordeCofig.zh==true and zh==true then
			zhonyas()
			CastSummonerBarrier()
			CastSummonerHeal()
		end
