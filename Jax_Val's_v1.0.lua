...
function JaxRun()
	...
	JaxConfig, menu = uiconfig.add_menu('Jax Config', 200)
...
function combo()
	if target ~= nil then
		...
		if timerreset == 0 and timerAA == 0 then
			if QRDY == 1 and GetDistance(target) > AArange and GetDistance(target) < 700 then
...
function Timer()
	if GetTickCount() - timerreset > (((delay+ping)/myHero.attackspeed)+ping*2) then
		timerreset = 0
	end
	...
	if targetaa ~= nil and timerAA ~= 0 then
...
function Cooldown_Handling()
	if myHero.SpellTimeQ > 1.0 and myHero.mana >= 60 then
	QRDY = 1
	else QRDY = 0
	end
	...
	if myHero.SpellTimeE > 1.0 and myHero.mana >= (65+(GetSpellLevel('E')*5)) then
	ERDY = 1
	else ERDY = 0
	end
