...
LissConfig:addParam("d", "Auto Dodge", SCRIPT_PARAM_ONKEYTOGGLE, false, 57)
...
--LissConfig:addParam('Qdelay', "Q Delay", SCRIPT_PARAM_NUMERICUPDOWN, 5.3, 189,0,10,0.1)
...
function Run()
	...
	if target~=nil then
		tqfx,tqfy,tqfz = GetFireahead(target,qdelay,23)
	...
	if targetQ~=nil then
			...
			tqfxQ,tqfyQ,tqfzQ = GetFireahead(targetQ,qdelay,23)
...
function autoQ()
	if targetQ~=nil and QRDY==1 then
		...
		elseif Passive==false and not LissConfig.qmp then
			if GetD(tqfaQ)<800 and CreepBlock(tqfxQ,tqfyQ,tqfzQ,50)==0 then
					CastSpellXYZ('Q',GetFireahead(targetQ,5.3,23))
			elseif GetD(tqfaQ)<800 and CreepBlock(tqfxQ,tqfyQ,tqfzQ,50)>0 then
					if getLine(tqfaQ,myHero) then
...
function getLine(object,obj)
	...
	for i = 20,dist,20 do
		...
		for _, minion in pairs(enemyMinions) do
			if minion~=nil then
				local tfx,tfy,tfz = GetFireahead(minion,qdelay,23)
