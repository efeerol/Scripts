...
function KhaRun()
	...
	targetaa = GetWeakEnemy('PHYS',myHero.range+50)
	Skillshots()
	...
	if myHero.SpellNameQ == "KhazixQQ" then
		Qrange = 325
	else
		Qrange = 375
	end
	
	if KhaConfig.combo then combo() end
	if KhaConfig.wskillhot then wskillhot() end
	if KhaConfig.ignite then ignite() end
	if KhaConfig.smite then smitesteal() end
	end
end

	KhaConfig = scriptConfig("KhaZix Config", "khaconf")
	KhaConfig:addParam("combo", "Combo", SCRIPT_PARAM_ONKEYDOWN, false, 88) -- X
	KhaConfig:addParam("wskillhot", "W-Skillhot", SCRIPT_PARAM_ONKEYDOWN, false, 89) -- Y
	KhaConfig:addParam("useItems", "Use Items", SCRIPT_PARAM_ONKEYTOGGLE, true, 97) -- Num1
	KhaConfig:addParam("ignite", "Use Ignite", SCRIPT_PARAM_ONKEYTOGGLE, false, 98) -- Num2
	KhaConfig:addParam("smite", "Smitesteal", SCRIPT_PARAM_ONKEYTOGGLE, false, 99) -- Num3
	KhaConfig:addParam("drawskillshot", "Draw Skillshots", SCRIPT_PARAM_ONKEYTOGGLE, false, 100) -- Num4
	KhaConfig:addParam("dodgekillshot", "Dodge Skillshots", SCRIPT_PARAM_ONKEYTOGGLE, false, 101) -- Num5
	KhaConfig:addParam("BlockInputDelay", "BlockInput Delay", SCRIPT_PARAM_NUMERICUPDOWN, 500, 102, 0, 2500, 250) -- Num6
	KhaConfig:permaShow("combo")
	KhaConfig:permaShow("wskillhot")
	KhaConfig:permaShow("useItems")
	KhaConfig:permaShow("ignite")
	KhaConfig:permaShow("smite")
	KhaConfig:permaShow("drawskillshot")
	KhaConfig:permaShow("dodgekillshot")	
	KhaConfig:permaShow("BlockInputDelay")

function combo()									
	if target ~= nil then
		if KhaConfig.useItems and GetDistance(target) < myHero.range+50 then
			UseAllItems(target)
		end
	
		if QRDY == 1 and GetDistance(myHero, target) < Qrange then
			CastSpellTarget("Q",target)
		end
		
		if WRDY == 1 and GetDistance(myHero, target) < 1000 and CreepBlock(GetFireahead(target,1.6,16)) == 0 then
			CastSpellXYZ("W", GetFireahead(target,1.6,17))
		end
...
function wskillhot()
	if target ~= nil then
		if WRDY == 1 and GetDistance(myHero, target) < 1000 and CreepBlock(GetFireahead(target,1.6,16)) == 0 then
			CastSpellXYZ("W", GetFireahead(target,1.6,16))
		end
	end
end
...
function dodgeaoe(pos1, pos2, radius)
	...
	if calc < radius and KhaConfig.dodgekillshot == true then
...
function dodgelinepoint(pos1, pos2, radius)
	...
	if perpendicular < radius and calc1 < calc4 and calc2 < calc4 and KhaConfig.dodgekillshot == true then
		if KhaConfig.combo == false and KhaConfig.wskillhot == false then
			if KhaConfig.BlockInputDelay ~= 0 then
				send.block_input(true, KhaConfig.BlockInputDelay)
			end
		MoveToXYZ(dodgex,0,dodgez)
		end
	end
...
function Skillshots()
	...
	if KhaConfig.drawskillshot == true then
		for i=1, #skillshotArray, 1 do
...
SetTimerCallback("KhaRun")
print("\nVal's Kha'Zix v"..version.."\n")