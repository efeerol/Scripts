...
local cc = 0
	...
	XinConfig, menu = uiconfig.add_menu('XinZhao Hotkeys', 200)
	menu.keydown('Combo', 'Combo', Keys.X)
	menu.checkbutton('AutoQ', 'Auto-Q', true)
	menu.checkbutton('AutoW', 'Auto-W', true)
...
function Main()
	if IsLolActive() then
		...
		Skillshots()
		GetCD()
		if XinConfig.Combo and timer==nil and dodgetimer==0 then Combo() end
	end
end

function Combo()
	if target~=nil then
		if ERDY==1 and GetDistance(target)>AArange then CastSpellTarget('E ',target) end
	end
	if targetaa~=nil then AttackTarget(targetaa) end
	if target~=nil and XinConfig.MouseMove then
		MoveTarget(target)
	else
		MoveMouse()
	end
...
function OnProcessSpell(unit, spell)
	if targetaa ~= nil then
		if unit ~= nil and spell ~= nil and unit.charName == myHero.charName then
			if (spell.name == "XinZhaoBasicAttack" or spell.name == "XinZhaoBasicAttack2" or spell.name == "XinZhaoCritAttack" or spell.name == "XenZhaoBattleCry" or spell.name == "XenZhaoThrust" or spell.name == "XenZhaoComboTarget" or spell.name == "XenZhaoSweep") and spell.target~=nil and spell.target.name == targetaa.name then
...
function OnDraw()
	if myHero.dead == 0 and XinConfig.DrawCircles then
...
function dodgeaoe(pos1, pos2, radius)
	...
	if calc < radius and DodgeConfig.DodgeSkillShotsAOE == true and GetCursorX() > xa and GetCursorX() < xb and GetCursorY() > ya and GetCursorY() < yb and timer==nil then
		if DodgeConfig.BlockSettings == 1 and XinConfig.Combo == false then
			dodgetimer = GetTickCount()
			send.block_input(true,DodgeConfig.BlockTime)
			MoveToXYZ(dodgex,0,dodgez)
		elseif DodgeConfig.BlockSettings == 2 or XinConfig.Combo then
			dodgetimer = GetTickCount()
			MoveToXYZ(dodgex,0,dodgez)
		end
	end
