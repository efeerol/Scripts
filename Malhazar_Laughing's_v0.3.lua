...
function MalzRun()
	...
	if MalzConfig.combo then combo() end
	if MalzConfig.Harass then Harass() end
	if MalzConfig.ignite then ignite() end
    if MalzConfig.PushFarm then PushFarm() end
	end
...
function Main()
        ...
        XXXConfig = scriptConfig("XXX Config", "XXXconf")
        XXXConfig:addParam("drawskillshot", "Draw Skillshots", SCRIPT_PARAM_ONOFF, true)
        XXXConfig:addParam("dodgeskillshot", "Dodge Skillshots", SCRIPT_PARAM_ONOFF, true)
        XXXConfig:permaShow("drawskillshot")
        XXXConfig:permaShow("dodgeskillshot")

MalzConfig = scriptConfig("Malz Config", "Malzconfg")
MalzConfig:addParam("combo", "Combo (X)", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("X"))
MalzConfig:addParam("Harass", "Harass (Z)", SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("Z"))
MalzConfig:addParam("PushFarm", "Push Farm (C)", SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("C"))
MalzConfig:addParam("useItems", "Use Items", SCRIPT_PARAM_ONKEYTOGGLE, true, 112)
MalzConfig:addParam("ignite", "Auto Ignite", SCRIPT_PARAM_ONKEYTOGGLE, true, 113)
MalzConfig:addParam("s", "Cast Speed", SCRIPT_PARAM_NUMERICUPDOWN, 4, 97, .5, 20, .5)
MalzConfig:addParam("a", "Projectile Speed", SCRIPT_PARAM_NUMERICUPDOWN, 99, 98, 1, 99, 1)
MalzConfig:permaShow("PushFarm")
MalzConfig:permaShow("combo")
MalzConfig:permaShow("Harass")
MalzConfig:permaShow("PushFarm")
MalzConfig:permaShow("useItems")
MalzConfig:permaShow("ignite")
...
function combo()
	if target ~= nil then
		if MalzConfig.useItems then
			UseAllItems(target)
		end
		if QRDY == 1 and GetDistance(myHero, target) < 700 then
		CastSpellXYZ('Q',GetFireahead(target,MalzConfig.s,MalzConfig.a))
		end		
		if ERDY == 1 and GetDistance(myHero, target) < 700 then
				CastSpellTarget('E', target)
		end
		if WRDY == 1 and GetDistance(myHero, target) < 700 then
			CastSpellTarget('W', target)
		end
		if RRDY == 1 and GetDistance(myHero, target) < 700 then
				CastSpellTarget('R', target)
				repeat until os.time() > timer + 2.5
		end
		end
	if target == nil and MalzConfig.combo then
		MoveToMouse()
	elseif MalzConfig.combo then
	end
end
...
function Harass()	
	if target2 ~= nil then
		if QRDY == 1 and GetDistance(myHero, target2) < 900 then
		CastSpellXYZ('Q',GetFireahead(target2,4,99))
end
end
end
...
function autoFarm()
	...
	local E = getDmg("E",minion,myHero)
		if minion.health < E*ERDY and GetDistance(myHero, minion) < 650 then
...
function dodgeaoe(pos1, pos2, radius)
        ...
        if calc < radius and XXXConfig.dodgeskillshot == true then
...
function dodgelinepoint(pos1, pos2, radius)
        ...
        if perpendicular < radius and calc1 < calc4 and calc2 < calc4 and XXXConfig.dodgeskillshot == true then
...
function Skillshots()
        ...
        if XXXConfig.drawskillshot == true then
                for i=1, #skillshotArray, 1 do
