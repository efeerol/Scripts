...
local show_allies=0
    ...
    NidaleeConfig = scriptConfig("Nidalee Config", "Nidalee")
    NidaleeConfig:addParam("Spear", "Spear at Mouse", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("Z"))
	NidaleeConfig:addParam("Combo", "Combo", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("X"))
    NidaleeConfig:addParam("AAQ", "AAreset", SCRIPT_PARAM_ONKEYTOGGLE, true, string.byte("7"))
	NidaleeConfig:addParam("AutoHeal", "Auto Heal", SCRIPT_PARAM_ONKEYTOGGLE, true, string.byte("C"))
    NidaleeConfig:addParam("ignite", "Auto-Ignite", SCRIPT_PARAM_ONKEYTOGGLE, true, string.byte("8"))
    NidaleeConfig:addParam("AutoSpear", "Toggle Spear", SCRIPT_PARAM_ONKEYTOGGLE, true, string.byte("6"))
    NidaleeConfig:addParam("AimSpear", "Aimed Spear", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("T"))
    NidaleeConfig:addParam("Potion", "Potion", SCRIPT_PARAM_ONKEYTOGGLE, true, string.byte("9"))
    NidaleeConfig:addParam("HpPotion", "Potion Toggle", SCRIPT_PARAM_ONOFF, true)
    NidaleeConfig:addParam("Flask", "Flask Toggle", SCRIPT_PARAM_ONOFF, true)
    NidaleeConfig:addParam("Biscuit", "Biscuit Toggle", SCRIPT_PARAM_ONOFF, true)
    NidaleeConfig:addParam("Elixir", "Elixir Toggle", SCRIPT_PARAM_ONOFF, true)
    NidaleeConfig:addParam("PotionValue", "Potion Trigger HP", SCRIPT_PARAM_NUMERICUPDOWN, 0.75, 103, 0, 1, .1)
    NidaleeConfig:addParam("FlaskValue", "Flask Trigger HP", SCRIPT_PARAM_NUMERICUPDOWN, 0.74, 100, 0, 1, .1)
    NidaleeConfig:addParam("BiscuitValue", "Biscuit Trigger HP", SCRIPT_PARAM_NUMERICUPDOWN, 0.60, 97, 0, 1, .1)
    NidaleeConfig:addParam("ElixirValue", "Elixir Trigger HP", SCRIPT_PARAM_NUMERICUPDOWN, 0.30, 96, 0, 1, .1)
    NidaleeConfig:addParam("drawskillshot", "Draw Skillshots", SCRIPT_PARAM_ONOFF, true)
    NidaleeConfig:addParam("dodgekillshot", "Dodge Skillshots", SCRIPT_PARAM_ONOFF, true)
    NidaleeConfig:addParam("BlockInputDelay", "BlockInput Delay", SCRIPT_PARAM_NUMERICUPDOWN, 500, 113, 0, 2500, 250)
    NidaleeConfig:permaShow("AutoSpear")
    NidaleeConfig:permaShow("AimSpear")
	NidaleeConfig:permaShow("Combo")
	NidaleeConfig:permaShow("AAQ")
	NidaleeConfig:permaShow("AutoHeal")
	NidaleeConfig:permaShow("Potion")
    NidaleeConfig:permaShow("ignite")
    NidaleeConfig:permaShow("BlockInputDelay")
...
function NidaleeRun()
    ...
    targetq = GetWeakEnemy('PHYS',1600)
    ...
    if Cougar == false then
        if NidaleeConfig.AimSpear and targetq ~= nil then
            javelin_toss()
            if target ~= nil then AttackTarget(target) end
        end
		if NidaleeConfig.Combo and target~=nil then
            CastSpellTarget("R", myHero)
        end
        if NidaleeConfig.AutoSpear and targetq~= nil then javelin_toss() end
        if NidaleeConfig.AutoHeal then Heal() end
    end
	if Cougar == true then
		if NidaleeConfig.Combo and target~=nil then
                UseAllItems(target)
		...
		if NidaleeConfig.Combo and target==nil then moveToMouse() end
	end
    if NidaleeConfig.ignite then ignite() end
    if NidaleeConfig.AAQ then AAreset() end
    if NidaleeConfig.Spear then Spear() end
end

function RedElixir()
	if NidaleeConfig.Potion and bluePill == nil then
		if iHero.health < iHero.maxHealth * NidaleeConfig.PotionValue and NidaleeConfig.HpPotion and GetClock() > wUsedAt + 15000 then
			usePotion()
			wUsedAt = GetTick()
		elseif iHero.health < iHero.maxHealth * NidaleeConfig.FlaskValue and NidaleeConfig.Flask and GetClock() > vUsedAt + 10000 then 
			useFlask()
			vUsedAt = GetTick()
		elseif iHero.health < iHero.maxHealth * NidaleeConfig.BiscuitValue and NidaleeConfig.Biscuit then
			useBiscuit()
		elseif iHero.health < iHero.maxHealth * NidaleeConfig.ElixirValue and NidaleeConfig.Elixir then
			useElixir()
		end
	end
...
function DetectCougar()
        if myHero.range < 200 then
            DrawText("Cougar Form",10,40,0xFF00EE00);
            Cougar = true
        else
...
function Draw()
    if Cougar == true then
        DrawCircleObject(myHero, 400, 1)
    else
        DrawCircleObject(myHero, 1500, 1)
    end
end

function moveToMouse()
    if Cougar == true and WRDY == true then
        MoveToXYZ(GetCursorWorldX(), GetCursorWorldY(), GetCursorWorldZ())
        moved = GetClock()
    if GetClock() - moved > 500 then
...
function findrecall()
        if GetTickCount() - recalltimer < 8000 and GetTickCount() - recalltimer > 0 then
                recall = true
        elseif GetTickCount() - recalltimer > 8000 then
                recall = false
                recalltimer = 0
        end
end
...
function OnCreateObj(obj)
        if (GetDistance(myHero, obj)) < 100 then
                if string.find(obj.charName,"TeleportHome") then
                        recalltimer = GetTickCount()
                end
        end
end
...
function Heal()
    if recall == false then
        if myHero.health < myHero.maxHealth * 0.70 then
        if ERDY == true and Cougar == false and myHero.mana > 60 then
...
function CastW()
    if WRDY == true and GetDistance(myHero, target) > 225 and GetDistance(myHero, target) < 325 then
		AttackTarget(target)
        CastSpellTarget("W",target)
		return 1
    elseif GetDistance(myHero, target) > 325 then
		AttackTarget(target)
		return 0
	end
		return 1
...
function CastE()
    if ERDY == true and GetDistance(myHero, target) < 375 then
        CastSpellTarget("E",target)
    end
end

function Attack()
    if QRDY == false and WRDY == false and ERDY == false and GetDistance(myHero, target) < 225 then
        AttackTarget(target)
    end
end

function OnProcessSpell(unit,spell)
        if spell.name == "nidalee_cougarBasicAttack" or spell.name == "Nidalee_CougarBasicAttack2" or spell.name == "Nidalee_CougarCritAttack" and QRDY == true and Cougar == true then
            herotimer = GetClock()
        end
end
 
function FindHit()
if targetaa ~= nil and Cougar == true and QRDY == true then
...
function AAreset()
        if (targettimer - herotimer > MinAttTime) and (targettimer - herotimer < MaxAttTime) then
                if targetaa ~= nil then
                        if QRDY == true and Cougar == true and GetDistance(myHero, targetaa) < 300 then
...
function Spear()
    if Cougar == true then
        CastSpellTarget("R", myHero)
    elseif Cougar == false then
        if QRDY == true and CreepBlock(myHero.x,0,myHero.z,GetCursorWorldX(),GetCursorWorldY(),GetCursorWorldZ()) == 0 then
            CastSpellXYZ("Q",mousePos.x,0,mousePos.z) else 
        end
    end
end
...
function javelin_toss()
    if QRDY == true and GetDistance(myHero, targetq) < 1500 and GetDistance(myHero, targetq) > 250 then
        local bq = bestcoords(targetq)
        local vts = hero_table[targetq.name][3]
        if CreepBlock(qx,qy,qz,115.0) == 0 and bq == 1  and vts:len() ~= 0 then
            CastSpellXYZ("Q",qx, qy, qz)
        elseif CreepBlock(qx,qy,qz,115.0) == 0 and bq == 1 and vts:len() == 0 then
            CastSpellXYZ("Q",qx, qy, qz)
        end
    end
end
...
function bestcoords(btarget)
    local x1,y1,z1 = GetFireahead(btarget,1,13)
    ...
    elseif vvt:len() == 0 and vst:len() < 1485 then
            qx=btarget.x
            qy=btarget.y
            qz=btarget.z
            return 1
...
function dodgeaoe(pos1, pos2, radius)
	...
	if calc < radius and NidaleeConfig.dodgekillshot == true then
...
function dodgelinepoint(pos1, pos2, radius)
	...
	if perpendicular < radius and calc1 < calc4 and calc2 < calc4 and NidaleeConfig.dodgekillshot == true then
        if NidaleeConfig.Combo == false then
			if NidaleeConfig.BlockInputDelay ~= 0 then
				send.block_input(true, NidaleeConfig.BlockInputDelay)
			end
		MoveToXYZ(dodgex,0,dodgez)
		end
	end
...
function Skillshots()
	...
	if NidaleeConfig.drawskillshot == true then
		for i=1, #skillshotArray, 1 do
