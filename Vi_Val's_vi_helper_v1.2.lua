...
local skillingOrder = {Vi = {Q,W,E,Q,Q,R,Q,E,Q,E,R,E,E,W,W,R,W,W},}

function ViRun()
	if IsChatOpen() == 0 and myHero.name == "Vi" then
	AArange = (myHero.range+(GetDistance(GetMinBBox(myHero)))+150)
	target = GetWeakEnemy('PHYS',900)
	targetaa = GetWeakEnemy('PHYS',AArange)
	
	if ViSettings.Autolevel then Autolevel() end
	if ViSettings.drawskillshots then Skillshots() end
...
--					or GetDistance(QPos,enemy)<=25) then
						CustomCircleXYZ(125,10,1,mx,enemy.y,mz)
...
function dodgelinepoint(pos1, pos2, radius)
	...
	if perpendicular < radius and calc1 < calc4 and calc2 < calc4 and ViSettings.dodgeskillshots then
...
function dodgeaoe(pos1, pos2, radius)
	...
	if ViSettings.dodgeskillshots and calc < radius then
		MoveToXYZ(dodgex,0,dodgez)
	end
end
