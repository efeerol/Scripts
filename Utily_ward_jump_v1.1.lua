...
local _registry = {}
local Champs = {}
Champs["Katarina"]={spell="E"}
Champs["Jax"]={spell="Q"}
Champs["LeeSin"]={spell="W"}
local SRDY=0
...
function WardRun()
	if Champs[myHero.name] and myHero.dead~=1 then	
		...
		if myHero["SpellTime"..Champs[myHero.name].spell] > 1.0 and GetSpellLevel(Champs[myHero.name].spell) > 0 then
			SRDY = 1
		else 
			SRDY = 0
        end
...
function ward(hx,hy,hz)
	if hx~=nil and hy~=nil and hz~=nil then
		...
		if SRDY==1 and (myHero.SpellNameW=="BlindMonkWOne" or LeeSin==false) and wardsuccess==true and lastWardObject~=nil and GetD(lastWardObject)<700 then
			run_every(0.5,JSpell, lastWardObject)
