...
local show_allies = 0
local skillingOrder = {Vayne = {Q,W,E,Q,Q,R,Q,W,Q,W,R,W,W,E,E,R,E,E}}
...
function Main()
	if myHero.name == "Vayne" then
		SetVariables()
		if VayneConfig.AutoCarry and IsChatOpen() == 0 then AutoCarry() end
		if VayneConfig.autolevel then Autolevel() end
		if VayneConfig.drawskillshots then Skillshots() end
	end
end

	VayneConfig, menu = uiconfig.add_menu('Vayne Config', 200)
	menu.keydown('AutoCarry', 'AutoCarry', Keys.X)
	menu.checkbutton('antigapclose', 'AntiGapcloser', true)
	menu.checkbutton('AutoQ', 'Auto-Q', true)
	menu.checkbutton('AutoE', 'Auto-E', true)
	menu.checkbutton('AutoR', 'Auto-R', true)
	menu.checkbutton('AutoItems', 'Auto-Items', true)
...
function SetVariables()
	target = GetWeakEnemy("PHYS",myHero.range + GetDistance(GetMinBBox(myHero))) 
	...
	if GetTickCount()-dodgetimer>500 then dodgetimer = 0 end
	
	if myHero.SpellTimeQ > 1.0 and GetSpellLevel('Q') ~= 0 and myHero.mana>30 then QRDY = 1
	else QRDY = 0 end
	if myHero.SpellTimeE > 1.0 and GetSpellLevel('E') ~= 0 and myHero.mana>90 then ERDY = 1
	else ERDY = 0 end
	if myHero.SpellTimeR > 1.0 and GetSpellLevel('R') ~= 0 and myHero.mana>110 then RRDY = 1
	else RRDY = 0 end
end

function OnCreateObj(object)
	if object.charName == 'vayne_basicAttack_mis.troy' or object.charName == 'vayne_critAttack_mis.troy' or object.charName == 'vayne_ult_mis.troy' then
		shotFired = false
...
function AutoCarry()
    CustomCircle(myHero.range + GetDistance(GetMinBBox(myHero)),2,4,myHero)
    if target ~= nil then
        if VayneConfig.AutoItems then UseAllItems(target) end
		if VayneConfig.AutoE then SpellTarget(E,ERDY*WillHitWall(target,400),myHero,target,550) end
        if GetTickCount() > (lastAttack + 275 / (myHero.attackspeed/(1/0.658))) then
			if ValidTarget(target) then
				...
				if VayneConfig.AutoR then SpellXYZ(R,QRDY+RRDY,myHero,target,myHero.range + GetDistance(GetMinBBox(myHero)),moveX,moveZ) end
				if VayneConfig.AutoQ then SpellXYZ(Q,QRDY,myHero,target,myHero.range + GetDistance(GetMinBBox(myHero)),moveX,moveZ) end
			end
			CustomCircle(100,10,1,target)
		else
			CustomCircle(100,5,2,target)
			if (shotFired == false or (GetTickCount() > (lastAttack + 275 / (myHero.attackspeed/(1/0.658))))) then
				local moveSqr = math.sqrt((mousePos.x - myHero.x)^2+(mousePos.z - myHero.z)^2)
				local moveX = myHero.x + 300*((mousePos.x - myHero.x)/moveSqr)
				local moveZ = myHero.z + 300*((mousePos.z - myHero.z)/moveSqr)
				if dodgetimer == 0 then MoveToXYZ(moveX,0,moveZ) end
			end
		end
    else
        local moveSqr = math.sqrt((mousePos.x - myHero.x)^2+(mousePos.z - myHero.z)^2)
...
function dodgeaoe(pos1, pos2, radius)
	...
	if VayneConfig.dodgeskillshots and calc < radius then
...
function dodgelinepoint(pos1, pos2, radius)
	...
	if perpendicular < radius and calc1 < calc4 and calc2 < calc4  and GetCursorX() > xa and GetCursorX() < xb and GetCursorY() > ya and GetCursorY() < yb then
		if VayneConfig.dodgeskillshots and VayneConfig.AutoCarry == false then 
...
print("\nVal's Dominion Vayne v"..version.."\n")