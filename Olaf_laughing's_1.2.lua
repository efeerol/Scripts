...
function OlafRun()
	...
	target6 = GetWeakEnemy('TRUE',325)
	minion = GetLowestHealthEnemyMinion(1000)
	targetignite = GetWeakEnemy('TRUE',600)
	if OlafConfig.combo then combo() end
	if OlafConfig.ULTI then ULTI() end
	if OlafConfig.Harass then Harass() end
	if OlafConfig.EHarass then EHarass() end
	if OlafConfig.ignite then ignite() end
    if OlafConfig.PushFarm then PushFarm() end
end
...
OlafConfig = scriptConfig("Olaf Config", "Olafconfg")
OlafConfig:addParam("combo", "Combo (X)", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("X"))
OlafConfig:addParam("Harass", "Harass (Z)", SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("Z"))
OlafConfig:addParam("EHarass", "E Harass (T)", SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("T"))
OlafConfig:addParam("PushFarm", "Push Farm (C)", SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("C"))
OlafConfig:addParam("useItems", "Use Items", SCRIPT_PARAM_ONKEYTOGGLE, true, 112)
OlafConfig:addParam("ignite", "Auto Ignite", SCRIPT_PARAM_ONKEYTOGGLE, true, 113)

OlafConfig:permaShow("PushFarm")
OlafConfig:permaShow("combo")
OlafConfig:permaShow("Harass")
OlafConfig:permaShow("EHarass")
OlafConfig:permaShow("useItems")
OlafConfig:permaShow("ignite")
...
function combo()
	if target4 ~= nil then
		if OlafConfig.useItems then 
			UseAllItems(target4) 
		end
		if QRDY == 1 then 
			CastHotkey("SPELLQ:WEAKENEMY RANGE=1000 FIREAHEAD=1.6,16 OVERSHOOT") 
		end
		SpellTarget(E,ERDY,myHero,target6,325)
		SpellTarget(W,WRDY,myHero,target5,150)
		SpellTarget(R,RRDY,myHero,target5,150)
	elseif target4 == nil and OlafConfig.combo then
		MoveToMouse()
	end
end
...
function autoFarm()
	if minion ~= nil then
		...
		CastHotkey("SPELLQ:WEAKMINION RANGE=1000 FIREAHEAD=1.6,16 OVERSHOOT")
	end
end
