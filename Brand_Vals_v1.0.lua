...
local Wdelay = 9
local Wspeed = 0
local skillingOrder = {Brand={Q,W,E,W,W,R,W,E,W,Q,R,Q,Q,Q,E,R,E,E},}

function Main()
	if IsChatOpen() == 0 and myHero.name == "Brand" then
		SetVariables()
	
		if BrandConfig.Wspell then Wspell() end
		if BrandConfig.Qspell then Qspell() end
		if BrandConfig.AutoLevel then AutoLevel() end
		if BrandConfig.drawskillshots then Skillshots() end
	end
end

	BrandConfig, menu = uiconfig.add_menu('Katarina Config', 250)
...
function Qspell()
	if target ~= nil then
		if QRDY == 1 and CreepBlock(GetFireahead(target,Qdelay,Qspeed)) == 0 then
			if ERDY == 1 and distXYZ(myHero.x,myHero.z,target.x,target.z)<Erange then
				SpellTarget(E,ERDY,myHero,target,Erange)
			end
		elseif QRDY == 0 and ERDY == 1 then
			SpellTarget(E,ERDY,myHero,target,Erange)
		end
		SpellPred(Q,QRDY,myHero,target,Qrange,Qdelay,Qspeed,1)
	end
	MoveToMouse()
end
...
function SetVariables()
	...
	target = GetWeakEnemy('MAGIC',Wrange,"NEARMOUSE")
		
	if myHero.SpellTimeQ>1.0 and GetSpellLevel('Q')~=0 and myHero.mana>50 then
	QRDY = 1
	else QRDY = 0
	end
	if myHero.SpellTimeW>1.0 and GetSpellLevel('W')~=0 and myHero.mana>(65+(myHero.SpellLevelW*5)) then
	WRDY = 1
	else WRDY = 0
	end
	if myHero.SpellTimeE>1.0 and GetSpellLevel('E')~=0 and myHero.mana>(65+(myHero.SpellLevelE*5)) then
	ERDY = 1
	else ERDY = 0
	end
...
function OnDraw()
	CustomCircle(Erange,4,2,myHero)
	CustomCircle(Wrange,4,3,myHero)
	if target ~= nil then
...
function dodgeaoe(pos1, pos2, radius)
	...
	if BrandConfig.dodgeskillshots and not spinning and calc < radius then
...
function dodgelinepoint(pos1, pos2, radius)
	...
	if perpendicular < radius and calc1 < calc4 and calc2 < calc4  and GetCursorX() > xa and GetCursorX() < xb and GetCursorY() > ya and GetCursorY() < yb then
		if BrandConfig.dodgeskillshots and not BrandConfig.Qspell and not BrandConfig.Wspell then 
