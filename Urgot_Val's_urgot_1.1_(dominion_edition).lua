...
local version = '1.1 (Dominion Edition)'
local Q,W,E,R = 'Q','W','E','R'
local metakey = SKeys.Control
local target,targetaa,GlobalTarget
local DomCap,RangeAttackQ = false,false
...
local show_allies = 0
local skillingOrder = {Urgot = {Q,W,E,Q,Q,R,Q,E,Q,E,R,E,E,W,W,R,W,W},}

function UrgotRun()
	if IsChatOpen() == 0 and tostring(winapi.get_foreground_window()) == "League of Legends (TM) Client" and myHero.name == "Urgot" then
	...
	if myHero.SpellTimeQ > 1.0 and GetSpellLevel('Q') > 0 and myHero.mana >= 40 then
	QRDY = 1
	else QRDY = 0
	end
	if myHero.SpellTimeW > 1.0 and GetSpellLevel('W') > 0 and myHero.mana >= (50+(GetSpellLevel('W')*5)) then
	WRDY = 1
	else WRDY = 0
	end
	if myHero.SpellTimeE > 1.0 and GetSpellLevel('E') > 0 and (45+(GetSpellLevel('E')*5)) then
	ERDY = 1
	else ERDY = 0
	end
	if myHero.SpellTimeR > 1.0 and GetSpellLevel('R') > 0 and myHero.mana >= 120 then
	RRDY = 1
	else RRDY = 0
	end
	...
	if UrgotConfig.QESpell then QESpell() end
	if UrgotConfig.AutoLevel then AutoLevel() end
	
end
end
	
	UrgotConfig, menu = uiconfig.add_menu('Urgot Config', 200)
	menu.keydown('Combo', 'Combo', Keys.X)
	menu.keydown('QESpell', 'Q- or E-Spell', Keys.Y)
	menu.keytoggle('AutoQ', 'Auto-Q', Keys.F1, true)
    menu.keytoggle('AutoE', 'Auto-E', Keys.F2, true)
	menu.keytoggle('AutoLevel', 'AutoLevel', Keys.F3, false)
	menu.permashow('Combo')
	menu.permashow('QESpell')
...
function Combo()
	...
	if target ~= nil then
		if GetDistance(myHero,target) < 900 and ERDY == 1 and UrgotConfig.AutoE then
			CastSpellXYZ('E',GetFireahead(target,1.6,15))
		end
		if GetDistance(target,myHero) < 900 and QRDY == 1 and CreepBlock(GetFireahead(target,1.6,16)) == 0 then
			CastSpellXYZ('Q',GetFireahead(target,1.6,16))
		end
		if WRDY == 1 and QRDY == 0 then
			CastSpellTarget('W',myHero)
		end
	end	
	if blockAndMove == nil or (blockAndMove ~= nil and not blockAndMove()) then
		if targetaa == nil then
			MoveToMouse()
		end
	end
...
function QESpell()
	if target ~= nil then
		if UrgotConfig.AutoE == false then
			if GetDistance(target,myHero) < 900 and ERDY == 1 then
				CastSpellXYZ('E',GetFireahead(target,1.6,15))
			end
		elseif UrgotConfig.AutoE == true then 
			if GetDistance(target,myHero) < 900 and QRDY == 1 and CreepBlock(GetFireahead(target,1.6,16)) == 0 then
				CastSpellXYZ('Q',GetFireahead(target,1.6,16))
			end
		end
...
function OnCreateObj(obj)
	if obj ~= nil then
		...
		if target ~= nil and string.find(obj.charName, "UrgotBasicAttack_mis") ~= nil and GetDistance(myHero, obj) < 200 and RangeAttackQ == true then
			CastSpellXYZ('Q',GetFireahead(target,1.6,16))
			if WRDY == 1 and QRDY == 0 then CastSpellTarget('W',myHero) end
			RangeAttackQ = false
		end
	end
end
...
function OnProcessSpell(unit,spell)
	if unit ~= nil and spell ~= nil and unit.charName == myHero.charName then
		if spell.name == "UrgotBasicAttack" or spell.name == "UrgotBasicAttack2" or spell.name == "UrgotCritAttack" and target ~= nil and targetaa ~= nil and spell.target ~= nil and spell.target.name == targetaa.name and QRDY == 1 and CreepBlock(GetFireahead(target,1.6,16)) == 0 and GetDistance(target,myHero) < 900 and UrgotConfig.AutoQ then
			RangeAttackQ = true
		end
	end
	...
	for i= 1,objManager:GetMaxHeroes(),1 do
		...
		if (h.team ~= myHero.team and h.visible==1 and h.invulnerable==0) then
			...
			if stacks==1 then
				...
				if (GetDistance(myHero,h)<1200) and DomCap == false then
					if WRDY == 1 then CastSpellXYZ('W',myHero.x,myHero.y,myHero.z) end
					if QRDY == 1 then CastSpellXYZ('Q',h.x,h.y,h.z) end
					break
				end
			end
		end
