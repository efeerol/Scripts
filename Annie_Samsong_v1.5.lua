...
function AnnieRun()
	...
	if AnnieConfig.harass then harass() end
	if AnnieConfig.ignite then ignite() end
	if AnnieConfig.autozhonya then autozhonya() end
end
local Summoners =
        {
...
AnnieConfig:addParam("harass", "Harass (Z)", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("Z"))
...
AnnieConfig:addParam("autozhonya", "Auto Zhonya", SCRIPT_PARAM_ONKEYDOWN, true, 116)
AnnieConfig:addParam("zhonyapercent", "Zhonya Percent", SCRIPT_PARAM_NUMERICUPDOWN, 15, 117, 5, 20, 1)
 
AnnieConfig:permaShow("combo")
AnnieConfig:permaShow("harass")
AnnieConfig:permaShow("useItems")
AnnieConfig:permaShow("ignite")
AnnieConfig:permaShow("autozhonya")
AnnieConfig:permaShow("zhonyapercent")

--[[Settings for Auto Dodge--]]
...
DodgeConfig, menu = uiconfig.add_menu("DodgeSkillshot Config", 250)
	...
	menu.slider("BlockSettings", "Block user input", 1, 3, 1, {"FixBlock","DynamicBlock","NoBlock"})
	menu.slider("BlockSettingsAOE", "Block user input for AOE", 1, 3, 3, {"FixBlock","DynamicBlock","NoBlock"})
...
function combo()
	if target ~= nil then
		...
		if CanCastSpell("Q") and GetDistance(myHero, target) < 620 then
...
function harass()
	...
	if target == nil and AnnieConfig.harass then
		MoveToMouse()
	elseif AnnieConfig.harass then
		AttackTarget(target)
	end
end
...
function autozhonya()
		if myHero.health < myHero.maxHealth*(AnnieConfig.zhonyapercent / 100) then
		useZhonyas()
		end
end
...
function CreateBlockAndMoveToXYZ(x, y, z)
    ...
    send.block_input(true,DodgeConfig.BlockTime,MakeStateMatch)
...
function dodgeaoe(pos1, pos2, radius)
	...
	if calc < radius and DodgeConfig.DodgeSkillShotsAOE == true and GetCursorX() > xa and GetCursorX() < xb and GetCursorY() > ya and GetCursorY() < yb then
		...
		elseif DodgeConfig.BlockSettingsAOE == 3 then
			MoveToXYZ(dodgex,0,dodgez)
		end
	end
end
...
function dodgelinepoint(pos1, pos2, radius)
	...
	if perpendicular < radius and calc1 < calc4 and calc2 < calc4 and DodgeConfig.DodgeSkillShots == true and GetCursorX() > xa and GetCursorX() < xb and GetCursorY() > ya and GetCursorY() < yb then
		...
		elseif DodgeConfig.BlockSettings == 2 then
			blockAndMove = CreateBlockAndMoveToXYZ(dodgex,0,dodgez)
		elseif DodgeConfig.BlockSettings == 3 then
			MoveToXYZ(dodgex,0,dodgez)
		end
	end
end
...
print("\nShow-/OP Annie by Samsong v"..version.."\n")
