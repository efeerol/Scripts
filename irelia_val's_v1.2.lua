...
function IreliaRun()
	...
	target2 = GetWeakEnemy('PHYS',myHero.range)
	targetignite = GetWeakEnemy('TRUE',600)
	minion = GetLowestHealthEnemyMinion(650)
	if IreliaConfig.Ultimate then Ultimate() end
	...
	if IreliaConfig.ignite then ignite() end
	end
end

	IreliaConfig = scriptConfig("Irelia Config", "ireliaconfig")
	IreliaConfig:addParam("Combo", "Combo", SCRIPT_PARAM_ONKEYDOWN, false, 88)
	IreliaConfig:addParam("Ultimate", "Ultimate", SCRIPT_PARAM_ONKEYDOWN, false, 89)
	IreliaConfig:addParam("AutoFarm", "AutoFarm", SCRIPT_PARAM_ONKEYTOGGLE, false, 112)
	IreliaConfig:addParam("AutoW", "Auto W", SCRIPT_PARAM_ONKEYTOGGLE, true, 113)
	IreliaConfig:addParam("AutoE", "Auto E", SCRIPT_PARAM_ONKEYTOGGLE, true, 114)
	IreliaConfig:addParam("killsteal", "Killsteal", SCRIPT_PARAM_ONKEYTOGGLE, true, 115)
	IreliaConfig:addParam("useItems", "Use Items", SCRIPT_PARAM_ONKEYTOGGLE, true, 116)
 	IreliaConfig:addParam("ignite", "Auto Ignite", SCRIPT_PARAM_ONKEYTOGGLE, true, 117)
	IreliaConfig:addParam("BlockInputDelay", "BlockInput Delay", SCRIPT_PARAM_NUMERICUPDOWN, 500, 118, 0, 2500, 250)
	IreliaConfig:addParam("drawskillshot", "Draw Skillshots", SCRIPT_PARAM_ONOFF, true)
	IreliaConfig:addParam("dodgekillshot", "Dodge Skillshots", SCRIPT_PARAM_ONOFF, true)
	IreliaConfig:permaShow("Combo")
	IreliaConfig:permaShow("Ultimate")
	IreliaConfig:permaShow("AutoFarm")
	IreliaConfig:permaShow("AutoW")
	IreliaConfig:permaShow("AutoE")
	IreliaConfig:permaShow("killsteal")
	IreliaConfig:permaShow("useItems")
	IreliaConfig:permaShow("ignite")
	IreliaConfig:permaShow("BlockInputDelay")

function Combo()
	if target ~= nil then
		if IreliaConfig.useItems then
			UseAllItems(target)
		end
		if GetDistance(myHero, target) < 650 and GetDistance(myHero, target) > myHero.range+50 and QRDY == 1 then
			CastSpellTarget("Q", target)
		end
		if GetDistance(myHero, target) < 425 and ERDY == 1 and IreliaConfig.AutoE and myHero.health < target.health then
...
function OnProcessSpell(unit,spell)
	if target ~= nil then
		if unit ~= nil and spell ~= nil and unit.charName == myHero.charName then
			if spell.name == "IreliaBasicAttack" or spell.name == "IreliaBasicAttack2" or spell.name == "IreliaCritAttack" and spell.target and IreliaConfig.AutoW and WRDY == 1 then
...
function Ultimate()
	if target ~= nil and RRDY == 1 and GetDistance(myHero,target) < 1000 then
		CastSpellXYZ('R',GetFireahead(target,1.6,20))
	end
end
...
function killsteal()
	if target ~= nil then
		...
		if target.health < Q*QRDY and GetDistance(myHero, target) < 650 then
			CastSpellTarget('Q', target)
		end
	end
end
...
function AutoFarm()
	...
	local Q = getDmg("Q",minion,myHero)
		if minion.health < Q*QRDY and GetDistance(myHero, minion) < 650 then
...
function dodgeaoe(pos1, pos2, radius)
	...
	if calc < radius and IreliaConfig.dodgekillshot == true then
...
function dodgelinepoint(pos1, pos2, radius)
	...
	if perpendicular < radius and calc1 < calc4 and calc2 < calc4 and IreliaConfig.dodgekillshot == true then
		if IreliaConfig.Combo == false and IreliaConfig.Ultimate == false then
			if IreliaConfig.BlockInputDelay ~= 0 then
				send.block_input(true, IreliaConfig.BlockInputDelay)
			end
		MoveToXYZ(dodgex,0,dodgez)
		end
	end
...
function Skillshots()
	...
	if IreliaConfig.drawskillshot == true then
		for i=1, #skillshotArray, 1 do
...
function LoadTable()
	...
	for i=0, iCount, 1 do
		...
		if skillshotplayerObj.name == "Jayce" then
...
SetTimerCallback("IreliaRun")
print("\nVal's Irelia v"..version.."\n")