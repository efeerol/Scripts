...
function Main()
	if IsChatOpen() == 0 and IsLolActive() then
		SetVariables()
		if WarwickMenu.Combo then Combo() end
		if WarwickMenu.Combo2 then Combo2() end
		if WarwickMenu.jungle then UpdatejungleTable1() KS1() end
		if WarwickMenu.AutoFarm then AutoFarm() end
		if WarwickMenu.Killsteal then Killsteal() end
		if WarwickMenu.Autoharass then Autoharass() end
		if WarwickMenu.ignite then ignite() end
		if WarwickMenu.AutoFarm and not Combo() and not Combo2() then AutoFarm() end
	end
	end
		
	WarwickMenu, menu = uiconfig.add_menu('Laughings Warwick 2.0', 200)
        ...
        if #jungle1 > 0 then
                for i, creep in pairs(jungle1) do
                        if creep.name == "AncientGolem" or creep.name == "GreatWraith" or creep.name == "Dragon" or creep.name == "Worm" or creep.name == "LizardElder" or creep.name == "YoungLizard" or creep.name == "SmallGolem" or creep.name == "GiantWolf" or creep.name == "Wolf" or creep.name == "Wraith" or creep.name == "LesserWraith" or creep.name == "Golem" then
								...
								if creep.hero.dead == 0 and GetDistance(creep.hero) <= 600 then --and creep.team ~= myteam
                                        if getDmg('Q',creep.hero,myHero) > creep.hero.health then                                
                                        --cfa={x=creep
...
function Combo()
	if target ~= nil then
		...
		SpellTarget(Q,QRDY,myHero,target,400)
	elseif target == nil and WarwickMenu.Combo then
		MoveToMouse()
	end
end
...
function Combo2()
	if target ~= nil then
		...
		SpellXYZ(W,WRDY,myHero,target,400,myHero.x,myHero.z)
		SpellTarget(Q,QRDY,myHero,target,400)
		SpellTarget(R,RRDY,myHero,target,700)
	elseif target == nil and WarwickMenu.Combo2 then
		MoveToMouse()
	end
end
...
function AutoFarm()
    ...
    function OnCreateObj(obj)
            if WarwickMenu.AutoCleanse then
